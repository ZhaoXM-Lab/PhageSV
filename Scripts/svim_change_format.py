...
def main():
    args=parseargs()
    # args.raw = '/home1/Laisenying/Data-analysis/projects/PhageSV/Test/Simulator/SimSV/Test2_1000/PBSim_reads/replicate2_6e5/svim/variants.vcf'
    # args.new = '/home1/Laisenying/Data-analysis/projects/PhageSV/Test/Simulator/SimSV/Test2_1000/PBSim_reads/replicate2_6e5/CAST/new_vcf/svim.reformat.vcf'
    p_vcf = args.raw
    new_vcf = args.new   
    ...
    for line in open(p_vcf,"r").readlines():  
        if line.startswith('#'):
            ...
            if scoreValue >= args.score and string_list[6] == 'PASS':
                ...
                if 'CUTPASTE' in info_list:
                    info_list.remove('CUTPASTE')                                                              
                info_dict = {}
                for i in info_list:
                    ni = i.split('=')
                    info_dict[ni[0]] = ni[1]   
                if info_dict['SVTYPE'] == 'BND':
                    continue
                if info_dict['SVTYPE'] == 'DUP:TANDEM':
                    info_dict['SVTYPE'] = 'DUP' 
                    string_list[4] = '<DUP>' 
                if info_dict['SVTYPE'] == 'DUP:INT':
                    info_dict['SVTYPE'] = 'DUP' 
                    string_list[4] = '<DUP>'                                                                                                
